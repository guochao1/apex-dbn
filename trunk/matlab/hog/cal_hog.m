function F = cal_hog(img, cellpw, cellph, nblockw, nblockh,...
...
%                               'localinterpolate', 'unsigned', 'l2-hys');
...
%       than mine without any doubt, ╮(╯▽╰)╭ . By the way, I have not
%       used this feature vector to do any other works like classification,
%       so does my realization has no bugs？ I think so..., the follow-up
%       works will be started next week, so the answer will be coming soon,
%       :-D.
%      
% Author:   timeHandle
% Time:     March 24, 2010
...
% calculate gradient scale metric.
hx = [-1,0,1];
hy = -hx';
...
% calculate gradient orientation metric.
...
% generate the metric hist3dbig for storing the 3-dimensions histogram. the
% metric covers the whole image in the 'globalinterpolate' condition or
% covers the local block in the 'localinterpolate' condition. The metric is
...
% generate the metric for storing histogram of one block;
...
% generated from the 'bigger' metric hist3dbig is the final histogram.
if glbalinter == 1
    ...
    for btlx = 1:xbstep:xbstridend
        for bi = 1:(cellph*nblockh)
            for bj = 1:(cellpw*nblockw)
                ...
                z1 = (binz1-1.5)*(or*pi/nthet) + 0.0087;
        ...
        if glbalinter == 0
            ...
            for ibin = 1:nblockh
                for jbin = 1:nblockw
                    idsF = nthet*((ibin-1)*nblockh+jbin-1)+1;
