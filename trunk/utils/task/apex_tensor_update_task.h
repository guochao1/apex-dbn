...
namespace apex_utils{
	...
	// gavinhu: model updater using labeled data
	template<typename T>
	class ITensorLabeledUpdater: public ITensorUpdater<T> {
	public:
		// update the model using a trunk of label and a trunk of data
		virtual void train_update_trunk(const T &label, const T &data) = 0;
		// validate the model using a trunk of label and a trunk of data
		virtual void validate_trunk(const T &label, const T &data) = 0;
	};

	// gavinhu: update task for labeled data
    template<typename T>
    class TensorLabeledUpdateTask : public ITask {
    private:
        ITensorLabeledUpdater<T>   *updater;
        ITensorIterator<T>         *data_iter;
		ITensorIterator<T>         *label_iter;
    private:
        int task;
        int silent, do_validation;
        int trunk_size;
        ...
        inline void set_param_inner( const char *name, const char *val ){
            ...
            if( !strcmp( name, "print_step") ) print_step = atoi( val );
            if( !strcmp( name, "trunk_size") ) trunk_size = atoi( val );
            label_iter->set_param( name, val );
			data_iter->set_param( name, val );
            updater->set_param( name, val );
        }
    ...
    private:
        inline void task_update(){
            ...
            if( do_validation ){
                clock_t valid_start = clock();
				updater->validate_trunk( label_iter->validation_trunk(), data_iter->validation_trunk() );
                valid_elapsed += (double)(clock() - valid_start)/CLOCKS_PER_SEC; 
            }
            
            for( int i = 0 ; i < num_round ; i ++ ){
                int sample_counter = 0;
                label_iter->before_first();
				data_iter->before_first();
				while( label_iter->next_trunk() && data_iter->next_trunk() ){
					updater->train_update_trunk( label_iter->trunk(), data_iter->trunk() );
                    if( ++ sample_counter  % print_step == 0 ){
                        if( !silent ){
                            printf("\r                                                               \r");
                            printf("round %8d:[%8d] %.3lf sec elapsed, %.3lf sec for validation", i , sample_counter*trunk_size, elapsed, valid_elapsed );
    ...
    public:
        TensorLabeledUpdateTask( ITensorLabeledUpdater<T> *updater, ITensorIterator<T> *label_iter, ITensorIterator<T> *data_iter ){
            this->updater = updater;
            this->label_iter = label_iter;
        ...
        virtual ~TensorLabeledUpdateTask(){}
    };
