...
namespace apex_rbm{
	...
	class Predictor{
		...
		private:
			...
			int size_softmax;
		private:
			inline void feed_forward(const TSTensor2D &soft_max, TTensor1D &param, const TTensor3D &W, const TTensor1D &h_bias ){
		...
		public:
			Predictor( const PREDModel &model, const vector<TSTensor2D> train ){
					...
					W = clone( model.W );
					size_softmax = train[ 0 ].y_max;
					for(int i = 0; i < (int)train.size(); ++ i){
						TTensor1D param; 
						param = alloc_like( model.h_bias );
